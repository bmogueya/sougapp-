  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-text">{t('modules_nav', 'Gestion des Modules')}</h1>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {modules.map((module) => {
            ...
            return (
              <div key={module.id} className="bg-surface rounded-2xl shadow-card border border-border overflow-hidden">
                <div className="p-6">
                  ...
                  <h3 className="text-lg font-bold text-text mb-1">{module.name}</h3>
                  <p className="text-muted text-sm mb-4 h-10">{module.description}</p>
                  <div className="flex items-center gap-2">
                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${
                      module.is_active ? 'bg-success/10 text-success' : 'bg-surface-2 text-muted'
                    }`}>
                ...
                <div className="bg-surface-2 border-t border-border p-4">
                  <button className="text-sm font-medium text-primary hover:text-primary-strong w-full text-center">
